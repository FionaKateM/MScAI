...
### My RMarkdown Exercise {#markdownTitle}

[Link to my markdown title](#markdownTitle)
...
- Some job adverts are duplicates, in my experience, does the data take this into account?
- Tools can be mutually exclusive and only one be used (eg. a programming language) and others are used in combination, such as AWS. 
- There is also no weighting to their level of importance
...
pie(percent, main="Pie chart of in demand data scientist tools", labels=skill, clockwise=FALSE)
```
...
- pie charts are often difficult to read the values, specifically if those values are close to each other.
...
- horizontal guidelines would help with readbility
- the point I mentioned before that some tools live independently of each other and some are combined is not well represented
- I would be interested to know what percentage of job adverts mention both Python and R, as these are the focus points for this data. I think the rest can be removed. 
...
```{r}
skills_percent <- c(64, 59, 50, 28, 26, 23, 22, 20, 20, 12)
skills_skill <- c("machine learning", "statistics", "research", "prediction", "visualisation", "recommendation", "optimisation", "deep learning", "natural language processing", "regression")
skills_df <- data.frame("skills"=skills_skill, "perc"=skills_percent)

# Generate the charts

# Pie chart
pie(skills_percent, main="Pie chart of in demand data scientist skills", labels=skills_skill)

# Bar chart
barplot(skills_percent, 
main="Bar chart of in-demand Data Scientist skills (Jan 2020)", 
sub = "Source: https://towardsdatascience.com/what-are-the-in-demand-skills-for-data-scientists-in-2020-a060b7f31b11)",
ylab="Percentage of listings that mention skill", 
names.arg=skills_skill, 
cex.names = 0.6,                                     # shrinks text size so fits on x-axis
col="lightpink")                                     # colour of the bars  
```
...
# Check dataframe looks as it should
View(skills_df)

# use ggplot2 to plot the same bar chart as before
ggplot(data=skills_df, aes(x=skills,y=perc),fill=skills) + geom_bar(stat = 'identity',width = 1)

# and a pie chart
ggplot(data=skills_df, aes(x="", y=perc, fill=skills)) + geom_bar(stat="identity", width=1) + coord_polar("y", start=0)
```
...
**Exercise 3.1:** Change the number of students to 200 and the recommendation rate to 0.2. What's my new estimated income? 2,400 UKP
...
paste("After tax Martin will earn: ", net_income) 
```
...
- It also makes the variables reusable, and easily changeable. For example if the tax rate changes then updating it everywhere in the code would be laborious, much better to update it in one place and everywhere references that variable.
...
- 32 observations and 11 variables (it says this in the 'Environment' box in the top right of R Studio)
...
```{r}
# the mean mpg for cars in df
mean(df$mpg)

# the lowest mpg for cars in df
min(df$mpg)
```
...
mode <- function(v) {
  ...
  # turn the vector into a frequency table sorted in decreasing order
...
barplot(table(worms$Vegetation), main = "Frequency of vegetation types")
```
...
# bar plot of the cut types in the diamonds dataset
barplot(summary(diamonds$cut), xlab = "Quality of the cut", ylab = "frequency", main = "Different diamond qualities in the diamonds dataset")

# bar plot of the colors in the diamonds dataset
barplot(summary(diamonds$color), xlab = "Diamond colour (best to worst)", ylab = "frequency", main = "Different diamond colours in the diamonds dataset")

# bar plot to show diamond clarity
barplot(summary(diamonds$clarity), xlab = "Clarity (worst to best)", ylab = "frequency", main = "Clarity of diamonds")

# boxplot to show value of diamonds in dataset
